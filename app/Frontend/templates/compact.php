	...
	">
		<div style="display: flex; align-items: center; gap: 12px;">
			...
			<input type="checkbox" class="gcow-bump-checkbox"
				...
				style="width: 18px; height: 18px; cursor: pointer; flex-shrink: 0; accent-color: <?php echo esc_attr( $styles['buttonColor'] ?? '#3b82f6' ); ?>;"
			/>
			<!-- Image -->
			<div style="width: 48px; height: 48px; flex-shrink: 0; border-radius: 6px; overflow: hidden; background-color: #f3f4f6; display: flex; align-items: center; justify-content: center;">
			...
			<div style="flex: 1; min-width: 0;">
				<div style="display: flex; align-items: center; justify-content: space-between;">
					<h4 style="color: <?php echo esc_attr( $styles['textColor'] ?? '#1f2937' ); ?>; font-size: 13px; font-weight: 600; margin: 0;"><?php echo esc_html( $bump_data['title'] ); ?></h4>
					<div style="text-align: right; flex-shrink: 0;">
						<div style="color: <?php echo esc_attr( $styles['priceColor'] ?? '#111827' ); ?>; font-size: 15px; font-weight: 700;"><?php echo wp_kses_post( $bump_data['discounted_price'] ); ?></div>
						<?php if ( $bump_data['discount_value'] > 0 ) : ?>
							<div style="color: <?php echo esc_attr( $styles['saveBadgeColor'] ?? '#059669' ); ?>; font-size: 11px; font-weight: 500;">
								Save <?php echo wp_kses_post( $bump_data['saved_price'] ); ?>
							</div>
						<?php endif; ?>
					</div>
				</div>
				<?php if ( ! empty( $bump_data['description'] ) ) : ?>
					<p style="color: <?php echo esc_attr( $styles['descriptionColor'] ?? '#9ca3af' ); ?>; font-size: 12px; margin: 2px 0 0;"><?php echo esc_html( $bump_data['description'] ); ?></p>
				<?php endif; ?>
			</div>
		</div>
